...
class ThrustCalibration:
    def __init__(self):
        # Load parameters
        self.time_average_interval = float(rospy.get_param("~time_interval", 1.0))
        self.vbat_min = float(rospy.get_param("~min_battery_voltage", 13.2))
        self.mass_kg = float(rospy.get_param("~mass_kg", 1.0))
        ...
        rospy.Subscriber("/thrust_calibrate_trigger", Bool, self.start_trigger_cb)

    def battery_voltage_cb(self, msg):
        if not self.is_recording:
            return

        self.volt_buf = np.append(self.volt_buf, np.array([msg.voltage]), axis=0)
        ...
        if (cur_t - self.last_records_t).to_sec() > self.time_average_interval:
            ...
            if self.volt_records.size != 0 and self.volt_records[-1] < self.vbat_min:
                self.is_recording = False
                self.cal_and_save_data()
                self.volt_records = np.array([])
                self.cmd_records = np.array([])
                rospy.loginfo("Stop recording (voltage below threshold).")
    ...
    def cal_and_save_data(self):
        ...
        filedir = os.path.join(script_dir, "thrust_data")
        if not os.path.exists(filedir):
            os.makedirs(filedir)
        filepath = os.path.join(filedir, "mass_{}_kg_data.csv".format(self.mass_kg))
        f = open(filepath, "a")
        writer = csv.writer(f)
        writer.writerow(
            (
        ...
        rospy.loginfo("Stored to " + filepath)
